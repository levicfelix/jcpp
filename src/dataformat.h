...
int last_index_row(int i, int n) {
    int count = 0;
    for (int l = 0; l <= i; ++l) {
        count += n - l;
    }
...
std::vector<std::vector<double>> vectorToMatrix(const std::vector<double>& neigh_vec, int ntypes) {
    std::vector<std::vector<double>> neigh_mat(ntypes, std::vector<double>(ntypes, 0.0));

    for (int i = 0; i < ntypes; ++i) {
        for (int j = i; j < ntypes; ++j) {
            neigh_mat[i][j] = neigh_vec[last_index_row(i, ntypes) - (ntypes - j)];
