...
std::vector<double> computeJintegral(std::unordered_map<int, Atom>& atomsDomain, 
    ...
    for (auto& entry : atomsDomain) {
        ...
        double w = (atom.ke+atom.pe)*energyUnits; // Review this for high T
        double Jstrain = w*dq[0];
        
        // Traction contribution
        // Displacement gradient tensor Du=F-I, where I is the identity matrix
        std::vector<std::vector<double>> Du(3, std::vector<double>(3, 0.0));
        Du[0][0] = atom.Fxx-1.0; Du[0][1] = atom.Fxy;     Du[0][2] = atom.Fxz;
        Du[1][0] = atom.Fyx;     Du[1][1] = atom.Fyy-1.0; Du[1][2] = atom.Fyz;
        Du[2][0] = atom.Fzx;     Du[2][1] = atom.Fzy;     Du[2][2] = atom.Fzz-1.0;
        
        // Atomic stress tensor
        std::vector<std::vector<double>> Str(3, std::vector<double>(3, 0.0));
        Str[0][0] = atom.sxx*stressUnits; Str[0][1] = atom.sxy*stressUnits; Str[0][2] = atom.sxz*stressUnits;
        Str[1][0] = atom.sxy*stressUnits; Str[1][1] = atom.syy*stressUnits; Str[1][2] = atom.syz*stressUnits;
        Str[2][0] = atom.sxz*stressUnits; Str[2][1] = atom.syz*stressUnits; Str[2][2] = atom.szz*stressUnits;
