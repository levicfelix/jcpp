...
struct Params {
    ...
    std::vector<std::vector<double>> neighborcutoff = stringToMatrix("3.1 3.1 3.1 3.1", numtypes, numtypes);
    std::vector<std::vector<double>> neighborscaling = stringToMatrix("1.0 1.0 1.0 1.0 ", numtypes, numtypes);
...
void readParams(const std::string& filename, Params& params) {
    ...
    if (!paramStrings["Dimensions"].empty()) {
        params.dimensions = std::stoi(paramStrings["Dimensions"]);
    }
    ...
    if (!paramStrings["NeighborCutoff"].empty()) {
        params.neighborcutoff = stringToMatrix(paramStrings["NeighborCutoff"], params.numtypes, params.numtypes);
    }
    if (!paramStrings["NeighborScaling"].empty()) {
        params.neighborscaling = stringToMatrix(paramStrings["NeighborScaling"], params.numtypes, params.numtypes);
    }
